...
} % TODO no time right now but later could compare to LCG MVMs.
...
In other words, all the work is done in the inversions $K_{|\bsth}^{-1}\tbr,K_{|\bsth}^{-1}\Ty$, which in turn rely on the MVM operation, discussed in Section~\ref{fast-mvm}. Since $K_{|\bsth}$ only enters computation as an MVM operator, the amount of memory consumed is dictated by its representation, which need not be dense. % this is a good transition paragraph, and motivates what follows well, but perhaps reworking the structure of the prior sections will make this (for lack of a better word) pipeline bottleneck mroe obvious? TODO any suggestions?
...
% TODO get rid of all the linear terms below; delay big-O discussion until "asymptotics"?
...
Here, experiments were carried out on ``random'' parameters with the same seed. % im not sure exactly what that means ("random parameters")? same seed? what is an experiment? might call it an application to simulated data? if we are desperate for space, make this a single sentence... instead of an itemized list?
...
\caption{Test outputs for the FX2007 dataset. COGP mean is black, with 95\% confidence intervals shaded in grey. LLGP mean is a solid red curve, with light green 95\% confidence intervals. Magenta points are in the training set, while blue ones are in the test set.} % TODO much better -- might also have 10 words on high level point: Predictions from LLGP on the test set are uniformly better across these three tasks. or something.
\label{fx2007-graph}
\end{center}
\vskip -0.2in
\end{figure*}
