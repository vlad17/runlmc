...
We propose Large Linear GPs (LLGPs), which circumvent the need for stationarity by using LMC's structure, enabling optimization of GP hyperparameters for multi-dimensional outputs and low-dimensional inputs. When applied to synthetic two-dimensional input and real time series data, we find our theoretical improvement relative to the current state of the art is realized with LLGP reducing training time while improving or maintaining predictive accuracy. By using a direct likelihood approximation rather than a variational one predictive variance estimates are significantly improved.
\end{abstract}
...
% TODO rerun graphs with axis labels
\section{Results}
\label{sec:results}
